...
It acts as a translator between two different interfaces, wrapping an 
existing class with a new interface to make it compatible with a client's 
expectations.

## The Problem
In software development, we often need to use third-party libraries or 
legacy code that we cannot modify. If their method signatures or data 
requirements do not match our modern system's architecture, we face an 
incompatibility gap.

## Our Example
Scenario: We are developing a modern Graphic Design application.
1. The Target Interface (ModernRectangle): 
Our system expects to draw rectangles by providing two points 
the Top-Left corner `(x1, y1)` and the Bottom-Right corner `(x2, y2)`.

2. The Adaptee (LegacyRectangle): 
We have a powerful legacy library, but it uses a different coordinate 
system. It requires the Top-Left corner `(x, y)`, followed by the Width 
...
- **Decoupling:** The client (our app) remains independent of the legacy library's 
