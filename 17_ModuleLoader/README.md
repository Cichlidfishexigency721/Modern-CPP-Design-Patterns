...
## Intent
The Module Loader design pattern refers to the architecture and mechanisms 
for dynamically discovering, loading, linking, and executing separate code 
units (modules or plugins) at runtime, rather than at compile-time.

## The Problem
Most C++ applications are statically linked. The compiler knows about all 
the classes and functions beforehand. However, modern systems (like game 
...
(a process called "Name Mangling"). Because different compilers mangle 
...
```bash
- dlopen()  : Loads a dynamic library (.so) into memory.
- dlsym()   : Looks up a memory address for a specific symbol (name).
- dlclose() : Unloads the library from memory.
- dlerror() : Returns human-readable error messages.
```

Since `dlsym()` expects standard, unmangled C names, we use a C++ feature 
...
1. **The Contract (IModule.h):**
An abstract C++ interface shared by both the Host and the Plugin.
...
Pointers with custom deleters) to domesticate the dangerous C API, ensuring 
that libraries are closed and modules are destroyed perfectly and automatically.

## CROSS-PLATFORM CONSIDERATIONS (POSIX vs. Windows)

Dynamic loading is not currently part of the standard C++ language (up to 
...
demonstrated here—exporting C-linkage factory functions to instantiate and 
destroy complex C++ objects across an ABI boundary—remains the universal 
standard for plugin systems worldwide.
